...
{	
	...
	private boolean showInfoFragment = false;
	

	private final int colorSignificant;
	private final int colorInsignificant;
	private final int colorTxComments;
	...
	public TransactionsListAdapter(final Context context, @Nonnull final Wallet wallet, final int maxConnectedPeers, final boolean showBackupWarning)
	{
		...
		colorTxComments = resources.getColor(R.color.fg_significant);
	...
	public void bindView(@Nonnull final View row, @Nonnull final Transaction tx)
	{
		...
		try
		{
			...
			// transaction comment
			final View rowExtend = row.findViewById(R.id.transaction_row_extend); // need to initialize before txcomment
			//final String txComment = WalletUtils.getTransactionTxcomment(tx);
			String txComment = tx.getTxcomment();
			
			final View txCommentView = row.findViewById(R.id.transaction_row_txcomment);
			txCommentView.setVisibility(View.GONE);
			
			if(txComment.length() > 0) 
			{   
				// limit to 240 characters
				if(txComment.length() > 240) { txComment = txComment.substring(0, 240); }
				
				txCommentView.setVisibility(View.VISIBLE);
				txComment = txComment.replaceAll("^text:", "");
	
				final TextView txCommentText = (TextView) row.findViewById(R.id.transaction_message);
				
				txCommentText.setText(txComment);
				txCommentText.setTextColor(colorTxComments);
			}
