...
public final class SendCoinsFragment extends SherlockFragment
{
	...
	private EditText transactionCommentView;
	...
	private void setTxCommentString(String txCommentStr) 
	{
		this.txComment = txCommentStr;
	}
	...
	private final class ReceivingAddressActionMode implements ActionMode.Callback
	{
		...
		private void handleClear()
		{
			...
			transactionCommentView.setText(null);
			receivingAddressView.setText(null);
			if (receivingStaticAddressView.length() > 0) {
	...
	public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState)
	{
		...
		// Edittext field for transaction comment handling. 
		final EditText transactionCommentView = (EditText) view.findViewById(R.id.send_coins_transaction_comment);
		InputFilter[] filterArray = new InputFilter[1];
		filterArray[0] = new InputFilter.LengthFilter(140);
		transactionCommentView.setFilters(filterArray);
			
		transactionCommentView.setOnEditorActionListener(new OnEditorActionListener()
		{
			@Override		
			public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {	
				if (actionId == EditorInfo.IME_ACTION_DONE) {
					setTxCommentString(transactionCommentView.getText().toString());
					return false;
				}
				return true;
			}
	...
	private void initStateFromIntentExtras(@Nonnull final Bundle extras)
	{
		...
		final String txComment = extras.getString(SendCoinsActivity.INTENT_EXTRA_TRANSACTION_COMMENT);
		...
		update(address, addressLabel, amount, bluetoothMac, txComment);
	}
	...
	private void initStateFromBitcoinUri(@Nonnull final Uri bitcoinUri)
	{
		...
		new StringInputParser(input)
		{
			...
			{
				update(address.toString(), addressLabel, amount, bluetoothMac, txComment);
			}
	...
	private void saveInstanceState(final Bundle outState)
	{
		...
		if ( txComment != null)
			outState.putString("transaction_comment", txComment);
	}
	...
	private void restoreInstanceState(final Bundle savedInstanceState)
	{
		...
		if (savedInstanceState.containsKey("transaction_comment"))
			 txComment = savedInstanceState.getString("transaction_comment");
	}
		...
		if (requestCode == REQUEST_CODE_SCAN)
		{
			if (resultCode == Activity.RESULT_OK)
			{
				...
				new StringInputParser(input)
				{
					...
					protected void bitcoinRequest(final Address address, final String addressLabel, final BigInteger amount, final String bluetoothMac)
					{
						SendCoinsActivity.start(activity, address != null ? address.toString() : null, addressLabel, amount, bluetoothMac, txComment != null ? txComment : null);
					}
	...
	private void handleGo()
	{
		...
		builder.append("text:");
		builder.append(txComment);
		
		final SendRequest sendRequest = SendRequest.to(validatedAddress.address, amount, builder.toString());
	...
	public void update(final String receivingAddress, final String receivingLabel, @Nullable final BigInteger amount,
			@Nullable final String bluetoothMac, @Nullable final String txComment)
	{
		try
		{
