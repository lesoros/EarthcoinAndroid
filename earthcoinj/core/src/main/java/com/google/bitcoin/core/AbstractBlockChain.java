...
public abstract class AbstractBlockChain {
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
      checkState(lock.isLocked());
		...
		int nTargetSpacing = CoinDefinition.TARGET_SPACING;
        int nTargetTimespan =  CoinDefinition.TARGET_TIMESPAN;
        int nInterval =(nTargetTimespan / nTargetSpacing);

         if (storedPrev.getHeight() == 0 || storedPrev.getHeight() == 1 || storedPrev.getHeight() == 3 )
        {
          return;
        }
       ...
       if(cursor == null) return;
        ...
        int nActualSpacing = (int) (prev.getTimeSeconds() - blockIntervalAgo.getTimeSeconds());
        //log.info("nActualSpacing before retarget: " + prev.getTimeSeconds() + "-" + blockIntervalAgo.getTimeSeconds() + " = " + nActualSpacing);
        ...
        long diffTar =  prev.getDifficultyTarget();
        int diffMultiply   = ((nInterval - 1) * nTargetSpacing + (2 * nActualSpacing));
        int diffDivide   = ((nInterval +1) * nTargetSpacing);
            
            

        BigInteger newDifficulty = Utils.decodeCompactBits(prev.getDifficultyTarget());
        newDifficulty = newDifficulty.multiply(BigInteger.valueOf(diffMultiply));
        newDifficulty = newDifficulty.divide(BigInteger.valueOf(diffDivide));
    ...
    private void checkDifficultyTransitions_original(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
      checkState(lock.isLocked());
        ...
        if (storedPrev.getHeight() == 1)
        { 
            return;
        }
        if (storedPrev.getHeight() == 2)
        {
            return;
        }
