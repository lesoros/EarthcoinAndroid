...
public class Transaction extends ChildMessage implements Serializable {
    ...
    void parse() throws ProtocolException {
        ...
        if(version > 1)                     //HashEngineering changes
        {
            txCommentStr = readStr();	
            optimalEncodingMessageSize +=  VarInt.sizeOf(txCommentStr.length()) + txCommentStr.length();
        }
        length = cursor - offset;
    }
    ...
    public void setTxcomment(String txCommentStr) {
        unCache();

        this.txCommentStr = txCommentStr;
    }
    
    public String getTxcomment() {
        maybeParse();
        return txCommentStr != null ? txCommentStr : "";
    }
